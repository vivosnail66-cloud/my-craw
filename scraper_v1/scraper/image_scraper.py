...
def scrape_media_task(task):
    # get task info
    _, media_url, file_name, media_type, _ = task
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if p.exists():
                ...
                update_ret = db_sqlite.update_image_status(media_url, settings.STATUS_SUCCESS)
                logger.info(f"Update {media_type} {file_name} status to success with ret {update_ret}")
...
def fix_all_image_urls():
    ...
    for result in results:
        ...
        ret = db_sqlite.execute_sql(sql)
        logger.info(f"Update review {review_id} images to {new_image_url} with ret {ret}")
    ...
    for result in results:
        ...
        for failed_image_url in failed_list:
            if failed_image_url in description:
                logger.info(f"Found failed image url {failed_image_url} in product {url} description.")
                # update image url
                # repalce failed image url with "https://img.bestvibe.com/cdn-cgi/image/format=auto//mobile/asset/css/f/blank.png"
                description = description.replace(failed_image_url, "https://img.bestvibe.com/cdn-cgi/image/format=auto//mobile/asset/css/f/blank.png")
                sql = f"UPDATE products SET description = ? WHERE sku = ? and url = ?;"
                ret = db_sqlite.execute_sql(sql, (description, sku, url))
                logger.info(f"Update product {url} description to {description} with ret {ret}")
